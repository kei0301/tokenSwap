...
export default function FormPropsTextFields() {
    ...
    useEffect(() => {
        ...
        for (let t = 0; t < keylist.length; t++) {
            ...
            // else if (age === element.address2) {
            //     if (age2 === element.address1) {
            //         setRate(1 / element.rate);
            //     }
            // }
        }
    }, [age, age2])
    ...
    const getBalance = async () => {
        if (age === "So11111111111111111111111111111111111111111") {
                            ...
                            connection.getTokenAccountBalance(result.value[0].pubkey)
                                .then(result => {
                                    if (result.value.uiAmount < amount1) {
                                        ...
                                        if (age2 === "So11111111111111111111111111111111111111111") {
                                            splTosol();
                                        } else {
                                            splTospl();
                                        }
            ...
            axios.get(`${process.env.REACT_APP_BASE_URL}/trending/getTokenData`)
                .then(async (result) => {
                    if (result) {
                        ...
                        if (result.data[0][0].address1 === "So11111111111111111111111111111111111111111") {
                            await connection.getBalance(wallet.publicKey)
                                .then(result => {
            ...
            <Box sx={{ flexGrow: 1, p: '0px 100px !important' }}>
                <Grid container spacing={2}>
                    <Grid item xs={12} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <div style={{ position: 'relative', width: '50%', display: 'flex', border: '1px solid #b90ee2', borderRadius: '5px' }}>
